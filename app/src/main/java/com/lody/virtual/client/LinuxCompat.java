...
public class LinuxCompat {
    public static void forgeProcDriver(boolean z) {
        if (Build.VERSION.SDK_INT >= 26 && !VirtualCore.get().is64BitEngine() && !FileUtils.canRead("/proc/stat")) {
            File file = new File(z ? VEnvironment.getProcDirectory64() : VEnvironment.getProcDirectory(), "stat");
            if (!file.exists()) {
                try {
                    forgeProcStat(file);
                } catch (IOException e) {
    ...
    private static void forgeProcStat(File file) throws IOException {
        ...
        FileUtils.writeToFile(sb.toString().getBytes(), file);
    }
}
