package com.alipay.sdk.widget;

import android.app.Activity;
import android.app.AlertDialog;
import android.content.Context;
import android.content.res.Resources;
import android.graphics.BitmapFactory;
import android.graphics.drawable.BitmapDrawable;
import android.graphics.drawable.ColorDrawable;
import android.graphics.drawable.Drawable;
import android.graphics.drawable.GradientDrawable;
import android.os.Bundle;
import android.os.Handler;
import android.os.Looper;
import android.text.TextUtils;
import android.view.View;
import android.view.Window;
import android.view.animation.LinearInterpolator;
import android.view.animation.RotateAnimation;
import android.widget.FrameLayout;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.TextView;
import com.tencent.smtt.sdk.TbsListener;
import java.io.ByteArrayInputStream;
import p110z1.C3961ay;
import p110z1.C4921cd;

/* renamed from: com.alipay.sdk.widget.a */
/* loaded from: classes.dex */
public class C0665a {

    /* renamed from: a */
    public static final String f344a = "正在加载";

    /* renamed from: b */
    public static final String f345b = "去支付宝付款";

    /* renamed from: c */
    public static final String f346c = "去支付宝授权";

    /* renamed from: d */
    public static String f347d = "";

    /* renamed from: e */
    private AlertDialogC0666a f348e;

    /* renamed from: f */
    private Activity f349f;

    /* renamed from: g */
    private String f350g;

    /* renamed from: h */
    private long f351h = -1;

    /* renamed from: i */
    private final int f352i = 1;

    /* renamed from: j */
    private final long f353j = 15000;

    /* renamed from: k */
    private boolean f354k = false;

    /* renamed from: l */
    private Handler f355l = new HandlerC0669d(this, Looper.getMainLooper());

    /* renamed from: com.alipay.sdk.widget.a$a */
    /* loaded from: classes.dex */
    public class AlertDialogC0666a extends AlertDialog {
        /* JADX INFO: Access modifiers changed from: protected */
        public AlertDialogC0666a(Context context) {
            super(context);
        }

        @Override // android.app.AlertDialog, android.app.Dialog
        protected void onCreate(Bundle bundle) {
            super.onCreate(bundle);
            setContentView(m25262a(getContext()));
            Window window = getWindow();
            if (window != null) {
                window.setBackgroundDrawable(new ColorDrawable(0));
            }
        }

        /* renamed from: a */
        private View m25262a(Context context) {
            LinearLayout linearLayout = new LinearLayout(context);
            FrameLayout.LayoutParams layoutParams = new FrameLayout.LayoutParams(-2, m25261a(context, 50.0f));
            layoutParams.gravity = 17;
            linearLayout.setOrientation(0);
            linearLayout.setLayoutParams(layoutParams);
            GradientDrawable gradientDrawable = new GradientDrawable();
            gradientDrawable.setColor(-450944201);
            gradientDrawable.setCornerRadius(m25261a(context, 5.0f));
            linearLayout.setBackgroundDrawable(gradientDrawable);
            ImageView imageView = new ImageView(context);
            LinearLayout.LayoutParams layoutParams2 = new LinearLayout.LayoutParams(m25261a(context, 20.0f), m25261a(context, 20.0f));
            layoutParams2.gravity = 16;
            layoutParams2.setMargins(m25261a(C0665a.this.f349f, 17.0f), m25261a(C0665a.this.f349f, 10.0f), m25261a(C0665a.this.f349f, 8.0f), m25261a(C0665a.this.f349f, 10.0f));
            imageView.setLayoutParams(layoutParams2);
            imageView.setScaleType(ImageView.ScaleType.FIT_CENTER);
            imageView.setImageDrawable(m25260a(context, C0665a.f347d, 480));
            RotateAnimation rotateAnimation = new RotateAnimation(0.0f, 359.0f, 1, 0.5f, 1, 0.5f);
            rotateAnimation.setRepeatCount(-1);
            rotateAnimation.setDuration(900L);
            rotateAnimation.setInterpolator(new LinearInterpolator());
            imageView.startAnimation(rotateAnimation);
            TextView textView = new TextView(context);
            textView.setText(TextUtils.isEmpty(C0665a.this.f350g) ? C0665a.f344a : C0665a.this.f350g);
            textView.setTextSize(16.0f);
            textView.setTextColor(-1);
            LinearLayout.LayoutParams layoutParams3 = new LinearLayout.LayoutParams(-2, -2);
            layoutParams3.gravity = 16;
            layoutParams3.setMargins(0, 0, m25261a(context, 17.0f), 0);
            textView.setLayoutParams(layoutParams3);
            linearLayout.addView(imageView);
            linearLayout.addView(textView);
            FrameLayout frameLayout = new FrameLayout(context);
            frameLayout.setLayoutParams(new FrameLayout.LayoutParams(-2, -2, 17));
            frameLayout.addView(linearLayout);
            return frameLayout;
        }

        /* renamed from: a */
        private Drawable m25260a(Context context, String str, int i) {
            Throwable th;
            ByteArrayInputStream byteArrayInputStream;
            try {
                byteArrayInputStream = new ByteArrayInputStream(C3961ay.m9836a(str));
                try {
                    BitmapFactory.Options options = new BitmapFactory.Options();
                    if (i <= 0) {
                        i = TbsListener.ErrorCode.TPATCH_VERSION_FAILED;
                    }
                    options.inDensity = i;
                    options.inTargetDensity = context.getResources().getDisplayMetrics().densityDpi;
                    BitmapDrawable bitmapDrawable = new BitmapDrawable(context.getResources(), BitmapFactory.decodeStream(byteArrayInputStream, null, options));
                    try {
                        byteArrayInputStream.close();
                        return bitmapDrawable;
                    } catch (Exception unused) {
                        return bitmapDrawable;
                    }
                } catch (Throwable th2) {
                    th = th2;
                    try {
                        C4921cd.m5618a(th);
                        return null;
                    } finally {
                        if (byteArrayInputStream != null) {
                            try {
                                byteArrayInputStream.close();
                            } catch (Exception unused2) {
                            }
                        }
                    }
                }
            } catch (Throwable th3) {
                th = th3;
                byteArrayInputStream = null;
            }
        }

        /* renamed from: a */
        private int m25261a(Context context, float f) {
            return (int) (f * (context == null ? Resources.getSystem() : context.getResources()).getDisplayMetrics().density);
        }
    }

    public C0665a(Activity activity) {
        this.f349f = activity;
    }

    public C0665a(Activity activity, String str) {
        this.f349f = activity;
        this.f350g = str;
    }

    /* renamed from: a */
    public void m25270a(boolean z) {
        this.f354k = z;
    }

    /* renamed from: a */
    public void m25271a(String str) {
        this.f350g = str;
    }

    /* renamed from: a */
    public String m25274a() {
        return this.f350g;
    }

    /* renamed from: b */
    public void m25269b() {
        Activity activity = this.f349f;
        if (activity != null) {
            activity.runOnUiThread(new RunnableC0667b(this));
        }
    }

    /* renamed from: c */
    public void m25267c() {
        Activity activity = this.f349f;
        if (activity != null) {
            activity.runOnUiThread(new RunnableC0668c(this));
        }
    }

    /* renamed from: d */
    public void m25265d() {
        this.f349f = null;
        this.f348e = null;
    }
}
