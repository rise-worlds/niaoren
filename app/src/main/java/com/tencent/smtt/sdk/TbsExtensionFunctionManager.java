...
public class TbsExtensionFunctionManager {
    ...
    /* renamed from: b */
    private static TbsExtensionFunctionManager f12916b;
    ...
    public static TbsExtensionFunctionManager getInstance() {
        if (f12916b == null) {
            synchronized (TbsExtensionFunctionManager.class) {
                if (f12916b == null) {
                    f12916b = new TbsExtensionFunctionManager();
                }
            }
        }
        return f12916b;
    ...
    public synchronized void initTbsBuglyIfNeed(Context context) {
        ...
        if (!this.f12917a) {
            if (!canUseFunction(context, BUGLY_SWITCH_FILE_NAME)) {
                TbsLog.m16531i("TbsExtensionFunMana", "bugly is forbiden!!");
                return;
            }
            if (TbsShareManager.isThirdPartyApp(context)) {
                str = TbsShareManager.m16958c(context);
            } else {
                File q = TbsInstaller.m16742a().m16678q(context);
                if (q == null) {
                    TbsLog.m16531i("TbsExtensionFunMana", "getTbsCoreShareDir is null");
                }
                if (q.listFiles() != null && q.listFiles().length > 0) {
                    str = q.getAbsolutePath();
                }
                TbsLog.m16531i("TbsExtensionFunMana", "getTbsCoreShareDir is empty!");
                return;
            }
            if (TextUtils.isEmpty(str)) {
                TbsLog.m16531i("TbsExtensionFunMana", "bugly init ,corePath is null");
                return;
            }
            File q2 = TbsInstaller.m16742a().m16678q(context);
            if (q2 == null) {
                TbsLog.m16531i("TbsExtensionFunMana", "bugly init ,optDir is null");
                return;
            }
            File file = new File(str, "tbs_bugly_dex.jar");
            ReflectionUtils.m16407a(new DexLoader(file.getParent(), context, new String[]{file.getAbsolutePath()}, q2.getAbsolutePath(), QbSdk.getSettings()).loadClass("com.tencent.smtt.tbs.bugly.TBSBuglyManager"), "initBugly", (Class<?>[]) new Class[]{Context.class, String.class, String.class, String.class}, context, str, String.valueOf(WebView.getTbsSDKVersion(context)), String.valueOf(WebView.getTbsCoreVersion(context)));
            this.f12917a = true;
            TbsLog.m16531i("TbsExtensionFunMana", "initTbsBuglyIfNeed success!");
        }
    }
        ...
        if (z) {
            if (!file.exists()) {
                try {
                    ...
                    TbsLog.m16533e("TbsExtensionFunMana", "setFunctionEnable,createNewFile fail:" + str);
                    e.printStackTrace();
                    return false;
                }
            }
            ...
            TbsLog.m16533e("TbsExtensionFunMana", "setFunctionEnable,file.delete fail:" + str);
            return false;
        }
        return true;
    }
