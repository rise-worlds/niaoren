...
public class TBSResources {
    ...
    /* renamed from: d */
    private static Bitmap m16836d(String str) {
        try {
    ...
    /* renamed from: c */
    public static String m16837c(String str) {
        String str2 = "*/*";
        String substring = str.substring(str.lastIndexOf(Consts.f23430h) + 1, str.length());
        if (TextUtils.isEmpty(substring)) {
            return str2;
        }
        String str3 = Consts.f23430h + substring;
        int i = 0;
        while (true) {
            String[][] strArr = f13129a;
            if (i >= strArr.length) {
                break;
            } else if (str3.equalsIgnoreCase(strArr[i][0])) {
                str2 = f13129a[i][1];
                break;
            } else {
                i++;
