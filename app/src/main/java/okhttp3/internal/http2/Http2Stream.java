...
public final class Http2Stream {
    ...
    public boolean isLocallyInitiated() {
        return this.connection.client == ((this.f14704id & 1) == 1);
    }
    ...
    public void writeHeaders(List<Header> list, boolean z) throws IOException {
        ...
        if (list != null) {
            ...
            this.connection.writeSynReply(this.f14704id, z3, list);
            if (z2) {
    ...
    public void close(ErrorCode errorCode) throws IOException {
        if (closeInternal(errorCode)) {
            this.connection.writeSynReset(this.f14704id, errorCode);
        }
    }

    public void closeLater(ErrorCode errorCode) {
        if (closeInternal(errorCode)) {
            this.connection.writeSynResetLater(this.f14704id, errorCode);
        }
    }
    ...
    private boolean closeInternal(ErrorCode errorCode) {
        synchronized (this) {
            ...
            notifyAll();
            this.connection.removeStream(this.f14704id);
            return true;
        }
    }
    ...
    public final class FramingSink implements Sink {
        ...
        private void emitFrame(boolean z) throws IOException {
            ...
            Http2Stream.this.writeTimeout.enter();
            try {
                Http2Stream.this.connection.writeData(Http2Stream.this.f14704id, z && min == this.sendBuffer.size(), this.sendBuffer, min);
            } finally {
                Http2Stream.this.writeTimeout.exitAndThrowIfTimedOut();
            }
        }
                ...
                if (!this.closed) {
                    if (!Http2Stream.this.sink.finished) {
                        if (this.sendBuffer.size() > 0) {
                            ...
                            Http2Stream.this.connection.writeData(Http2Stream.this.f14704id, true, null, 0L);
                        }
                    }
                    synchronized (Http2Stream.this) {
                        this.closed = true;
