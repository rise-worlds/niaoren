    ...
    private void queueEvent(MailEvent mailEvent, Vector vector) {
        synchronized (this.qLock) {
            if (this.f14648q == null) {
                this.f14648q = new EventQueue();
            }
        }
        this.f14648q.enqueue(mailEvent, (Vector) vector.clone());
    }
    ...
    private void terminateQueue() {
        synchronized (this.qLock) {
            if (this.f14648q != null) {
                Vector vector = new Vector();
                vector.setSize(1);
                this.f14648q.enqueue(new TerminatorEvent(), vector);
                this.f14648q = null;
            }
        }
    }
