            ...
            if (hca.getPoolEntry() == null || hca.getManager() == this) {
                try {
                    try {
                        ...
                        Socket socket = entry2.getConnection().getSocket();
                        if (socket != null) {
                    ...
                    BasicPoolEntry entry3 = (BasicPoolEntry) hca.getPoolEntry();
                    boolean reusable2 = hca.isMarkedReusable();
                    hca.detach();
                    if (entry3 != null) {
                        this.connectionPool.freeEntry(entry3, reusable2, validDuration, timeUnit);
                    }
                    throw th;
                }
            } else {
