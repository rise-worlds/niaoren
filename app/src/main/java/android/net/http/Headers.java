...
public final class Headers {
    ...
    private static final String[] sHeaderNames = {TRANSFER_ENCODING, CONTENT_LEN, "content-type", CONTENT_ENCODING, CONN_DIRECTIVE, LOCATION, PROXY_CONNECTION, WWW_AUTHENTICATE, PROXY_AUTHENTICATE, CONTENT_DISPOSITION, ACCEPT_RANGES, "expires", CACHE_CONTROL, LAST_MODIFIED, "etag", SET_COOKIE, PRAGMA, "refresh", X_PERMITTED_CROSS_DOMAIN_POLICIES};
    ...
    static boolean containsIgnoreCaseTrimmed(CharArrayBuffer buffer, int beginIndex, String str) {
        ...
        boolean ok2 = ok;
        for (int j = 0; ok2 && j < size; j++) {
            char a = chars[beginIndex + j];
            char b = str.charAt(j);
            if (a != b) {
                ok2 = Character.toLowerCase(a) == Character.toLowerCase(b);
            }
        }
        return true;
    }
