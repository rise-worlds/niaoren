...
public class Analyzer {
    ...
    private static int getMaxDimensionTraversal(ConstraintWidget constraintWidget, int i, boolean z, int i2) {
        ...
        while (it.hasNext()) {
            i13 = Math.max(i13, getMaxDimensionTraversal(((ResolutionAnchor) it.next()).myAnchor.mOwner, i, z, i15));
        }
        int i16 = 0;
        for (Iterator<ResolutionNode> it2 = constraintWidget.mListAnchors[i7].getResolutionNode().dependents.iterator(); it2.hasNext(); it2 = it2) {
            i16 = Math.max(i16, getMaxDimensionTraversal(((ResolutionAnchor) it2.next()).myAnchor.mOwner, i, z, width + i15));
        }
        if (z2) {
            i13 -= i6;
            i9 = i16 + i5;
            ...
            i9 = i16 + ((i == 0 ? constraintWidget.getWidth() : constraintWidget.getHeight()) * i8);
            i10 = 1;
        }
        if (i == i10) {
            ...
            while (it3.hasNext()) {
                ...
                if (i8 == i10) {
                    i17 = Math.max(i17, getMaxDimensionTraversal(resolutionAnchor.myAnchor.mOwner, i, z, i6 + i15));
                    i7 = i7;
                } else {
                    i7 = i7;
                    i17 = Math.max(i17, getMaxDimensionTraversal(resolutionAnchor.myAnchor.mOwner, i, z, (i5 * i8) + i15));
                }
                it3 = it3;
                i10 = 1;
            }
            i11 = i7;
            i12 = (constraintWidget.mBaseline.getResolutionNode().dependents.size() <= 0 || z2) ? i17 : i8 == 1 ? i17 + i6 : i17 - i5;
        } else {
            i11 = i7;
            i12 = 0;
        }
        int max = margin + Math.max(i13, Math.max(i9, i12));
        int i18 = i15 + width;
        if (i8 == -1) {
            i15 = i18;
            i18 = i15;
        ...
        if (z) {
            Optimizer.setOptimizedWidget(constraintWidget, i, i15);
            constraintWidget.setFrame(i15, i18, i);
        } else {
            constraintWidget.mBelongingGroup.addWidgetsToSet(constraintWidget, i);
            constraintWidget.setRelativePositioning(i15, i);
        }
    ...
    private static int resolveDimensionRatio(ConstraintWidget constraintWidget) {
        ...
        if (constraintWidget.getHorizontalDimensionBehaviour() == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
            if (constraintWidget.mDimensionRatioSide == 0) {
                i2 = (int) (constraintWidget.getHeight() * constraintWidget.mDimensionRatio);
            } else {
                i2 = (int) (constraintWidget.getHeight() / constraintWidget.mDimensionRatio);
            }
            ...
            if (constraintWidget.mDimensionRatioSide == 1) {
                i = (int) (constraintWidget.getWidth() * constraintWidget.mDimensionRatio);
            } else {
                i = (int) (constraintWidget.getWidth() / constraintWidget.mDimensionRatio);
            }
