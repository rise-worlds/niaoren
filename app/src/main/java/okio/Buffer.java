...
public final class Buffer implements Cloneable, ByteChannel, BufferedSink, BufferedSource {
    ...
    public final Buffer readFrom(InputStream inputStream) throws IOException {
        readFrom(inputStream, cjm.f20759b, true);
        return this;
    }
    ...
    /* JADX WARN: Removed duplicated region for block: B:43:0x00ac A[EDGE_INSN: B:43:0x00ac->B:37:0x00ac ?: BREAK  , SYNTHETIC] */
    @Override // okio.BufferedSource
    /*
    ...
    */
    ...
    public String readUtf8LineStrict() throws EOFException {
        return readUtf8LineStrict(cjm.f20759b);
    }
    ...
    public String readUtf8LineStrict(long j) throws EOFException {
        if (j >= 0) {
            ...
            throw new EOFException("\\n not found: limit=" + Math.min(size(), j) + " content=" + buffer.readByteString().hex() + Typography.f21039E);
        }
            ...
            while (i < i2) {
                ...
                if (charAt < 128) {
                    ...
                    if (charAt > 56319 || charAt3 < 56320 || charAt3 > 57343) {
                        ...
                        writeByte((i7 >> 18) | TbsListener.ErrorCode.TPATCH_VERSION_FAILED);
    ...
    public long indexOf(byte b) {
        return indexOf(b, 0L, cjm.f20759b);
    }

    @Override // okio.BufferedSource
    public long indexOf(byte b, long j) {
        return indexOf(b, j, cjm.f20759b);
    }
    ...
    public long indexOf(byte b, long j, long j2) {
        ...
        if (j4 - j < j) {
            ...
            while (true) {
                j4 = (segment.limit - segment.pos) + j4;
                if (j4 >= j) {
                    break;
                }
                segment = segment.next;
    ...
    public long indexOf(ByteString byteString, long j) throws IOException {
        if (byteString.size() != 0) {
            ...
            if (j >= 0) {
                ...
                if (j2 - j >= j) {
                    while (true) {
                        j2 = (segment.limit - segment.pos) + j2;
                        if (j2 >= j) {
                            break;
                        }
                        segment = segment.next;
                ...
                long j4 = 1 + (this.size - size);
                long j5 = j;
                while (j2 < j4) {
                    byte[] bArr = segment.data;
                    int min = (int) Math.min(segment.limit, (segment.pos + j4) - j2);
                    for (int i = (int) ((segment.pos + j5) - j2); i < min; i++) {
                        if (bArr[i] == b) {
                            bArr = bArr;
                            segment = segment;
                            if (rangeEquals(segment, i + 1, byteString, 1, size)) {
                    ...
                    j5 = (segment.limit - segment.pos) + j2;
                    segment = segment.next;
                    j2 = j5;
                    j3 = -1;
                }
    ...
    private ByteString digest(String str) {
        try {
            ...
            return ByteString.m14867of(instance.digest());
    ...
    private ByteString hmac(String str, ByteString byteString) {
        try {
            ...
            return ByteString.m14867of(instance.doFinal());
